 ...
 *
 * @typedef {Record<string, string>} VerificationPayload
 ...
 *
 * @param {string|number|boolean|null|Record<string, string|number|boolean|null>} errors Validation errors keyed by form field.
 ...
 *
 * @param {Record<string, string|number|boolean|null|undefined>} body Submitted form fields to validate and apply.
 ...
 * @param {(token: string) => VerificationPayload} verifyToken (...args: never[]) => void that verifies and decodes the token.
 * @returns {AuthResult<{payload: VerificationPayload}>} Verified claims or a user-facing token error.
 */
function readVerificationToken(token, verifyToken) {
 ...
 * @param {string} userId Identifier of the user encoded in the verification link.
 * @param email Email address associated with the account.
 * @returns {import('express').RequestHandler} Authentication middleware.
 * @example
 * makeVerificationToken(userId, email);
 */
 ...
 * @returns {VerificationPayload|null} Verified token payload, or null when the token is invalid.
 */
 ...
 * @param {import('@mikro-orm/core').EntityManager} database MikroORM entity manager used to query and persist application records.
 * @param email Email address associated with the account.
 * @param {string} password Plain-text password supplied for credential verification or hashing.
 * @example
 * loginUser(database, email, password);
 * @returns {Promise<import('../models/User.js').User|null>} Authenticated user, or null when the credentials do not match.
 */
 ...
 * @param introHtml HTML content displayed before the verification action.
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 * sendVerificationEmail(user, introHtml);
 */
 ...
 * @param {import('@mikro-orm/core').EntityManager} database MikroORM entity manager used to query and persist application records.
 * @param name Display name used to personalize the generated content.
 * @param {string} email Email address associated with the account.
 * @param password Plain-text password supplied for authentication or replacement.
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 ...
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 * requestPasswordReset(database, email);
 */
 ...
 * @param {import('@mikro-orm/core').EntityManager} database MikroORM entity manager used to query and persist application records.
 * @param token Token submitted to verify the requested account action.
 * @param {string} email Email address associated with the account.
 * @param password Plain-text password supplied for authentication or replacement.
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 * resetUserPassword(database, token, email, password);
 */
 ...
 * @param {import('@mikro-orm/core').EntityManager} database MikroORM entity manager used to query and persist application records.
 * @param payload Data delivered to the registered handler.
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 * verifyUserEmail(database, payload);
 */
 ...
 * @param {import('@mikro-orm/core').EntityManager} database MikroORM entity manager used to query and persist application records.
 * @param body Submitted form fields to validate and apply.
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 * attemptLogin(database, body);
 */
 ...
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 * attemptForgotPassword(database, body);
 */
 ...
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 * attemptResetPassword(database, body);
 */
 ...
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 * attemptVerifyEmail(database, token);
 */
export async function attemptVerifyEmail(database, token) {
    const verification = readVerificationToken(token, verifyVerificationToken);
 ...
 * @returns {Promise<void>} Status payload describing whether a verification email was sent.
 */
...
/** Validates and persists editable profile fields for an authenticated user.
 * @param {import('@mikro-orm/core').EntityManager} database MikroORM entity manager used to query and persist application records.
 * @param {import('../models/User.js').User} user Authenticated user whose account data is being processed.
 * @param {Record<string, string|number|boolean|null|undefined>} body Submitted form fields to validate and apply.
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 * updateUserProfile(database, user, body);
 */
export async function updateUserProfile(database, user, body) {
    ...
    account.name = String(result.data.name);
    account.email = email;
    if (emailChanged) account.emailVerifiedAt = null;
    await database.flush();
    if (emailChanged) await sendVerificationEmail(account, '<p>Please verify your new email address.</p>');
    return { data: { user: account, status: emailChanged ? 'Profile updated. Please verify your new email address.' : 'Profile updated.' }, errors: null };
}

/** Validates the current password and persists a replacement password hash.
 * @param {import('@mikro-orm/core').EntityManager} database MikroORM entity manager used to query and persist application records.
 * @param {import('../models/User.js').User} user Authenticated user whose account data is being processed.
 * @param {Record<string, string|number|boolean|null|undefined>} body Submitted form fields to validate and apply.
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 * updateUserPassword(database, user, body);
 */
export async function updateUserPassword(database, user, body) {
    ...
    const result = validate(body, {
        ...
        [passwordField]: ['required', 'min:8', 'confirmed'],
    });
    if (!result.valid) return { data: null, errors: result.errors };
    if (!(await hash.check(String(result.data.current_password), account.password))) {
        return { data: null, errors: { current_password: ['Current password is incorrect'] } };
    }
    account.password = await hash.make(String(result.data[passwordField]));
    await database.nativeDelete(Session, { user_id: account.id });
    await database.flush();
    return { data: { status: 'Password updated. Please sign in again.' }, errors: null };
}

/** Validates the current password and removes the authenticated account.
 * @param {import('@mikro-orm/core').EntityManager} database MikroORM entity manager used to query and persist application records.
 * @param {import('../models/User.js').User} user Authenticated user whose account data is being processed.
 * @param {Record<string, string|number|boolean|null|undefined>} body Submitted form fields to validate and apply.
 * @returns {Promise<import('express').Response|void>} Promise resolving after the response is sent.
 * @example
 * deleteUserAccount(database, user, body);
 */
