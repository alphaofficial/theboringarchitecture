...
See [ARCHITECTURE.md](ARCHITECTURE.md) for project rules and patterns.
...
`config/pages.js` is autogenerated from `app/views/pages/**/*.jsx`. Add a `.jsx` page and the page manifest updates automatically through `npm run pages:generate`, `npm run build`, or the `pages:watch` process in development.
...
- `start:dev` runs four concurrent processes — `pages:watch` regenerates `config/pages.js`, `start:dev:server` runs nodemon, `start:dev:client` emits `public/app.js` and `public/main.css`, and `start:dev:ssr` emits `.ssr/ssr.mjs`.
...
- **`SSR: page not found: <Name>` in the logs.** The page name passed to `res.render` doesn't match any file under `app/views/pages/`. Check `config/pages.js` and the casing — Inertia page names are case-sensitive.
...
`config/pages.js` is regenerated automatically by `bin/generate-pages.sh`, which walks `app/views/pages/**/*.jsx`. Drop a new `Posts.jsx` and:
