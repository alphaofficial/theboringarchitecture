...
- **Mailer** — log + SMTP drivers; configured explicitly in `src/mail/config.ts`; send mail with `Mailer.send(to, subject, html)`
...
- **File Storage** — local + S3 drivers; `Storage.put / get / delete / url`; configured explicitly in `src/storage/config.ts`
...
├── events/           # Event driver config, app event registry, and handlers
├── jobs/             # Graphile Worker driver config, registry, and handlers
├── logger/           # Pino logger instance and configuration
├── mail/             # Mail config, templates, log + SMTP transport implementations
├── middleware/       # auth, sessions, errorHandler, rateLimit, inertia
├── models/           # Domain classes (User, Session)
├── primitives/       # Framework primitives and driver port interfaces
├── router/           # Express router
├── runtime/          # Explicit app driver composition
├── storage/          # Storage config and driver implementations
...
| `MAIL_FROM`                     | `noreply@example.com`   | Sender address used by `Mailer.send`; driver selection is explicit in code (`src/mail/config.ts`) |
| `STORAGE_PATH`                  | `storage`               | Default local storage root used by `LocalDiskDriver` in `src/storage/config.ts`               |
| `DATABASE_URL`                  | _(none)_                | PostgreSQL connection URL — required to enable the Queue (Graphile Worker)                    |
| `SCHEDULER_ENABLED`             | `false`                 | Reserved for application-specific scheduler toggles; the built-in scheduler script does not read this flag |
...
await Queue.dispatch('sendWelcomeEmailJob', { to: 'user@example.com', name: 'Alice' });
```

Job handlers live in `src/jobs/handlers/` and are registered from `src/jobs/jobs.ts`:
...
`sendWelcomeEmailJob` is a standalone queue example. The registration flow sends its verification email directly from `AuthController`, so the built-in `user.registered` event handler does **not** dispatch this job and will not send a duplicate welcome/verification email.

`src/worker.ts` imports the `jobs` map and passes it to `Queue.start(connectionString, jobs)`. Start the worker with:
...
The framework app composition explicitly injects the log transport by default in `src/mail/config.ts`. It does not switch mail drivers from environment variables:

| Driver | Description                                              |
| ------ | -------------------------------------------------------- |
| `log`  | Writes mail to the application log (default, no config) |

The SMTP transport remains available for applications that choose to wire it explicitly at boot time:
...
> **Note:** The built-in scheduler is script-driven/code-driven: `npm run scheduler` runs `src/scheduler.ts`, configures the scheduler driver, and registers the session cleanup schedule. `SCHEDULER_ENABLED` is not read by the built-in scheduler code.

## Events

A typed in-process event bus backed by Node's `EventEmitter`. App events (`user.registered`, `user.login`, `user.verified`) are defined in the application-owned event map at `src/events/events.ts`. Add application events there, not in primitives:
...
// Register the primitive driver explicitly at app boot.
Emitter.setDriver<AppEvents>(new NodeEventDriver<AppEvents>());

// Listen
Emitter.on<AppEvents, 'order.placed'>('order.placed', ({ orderId, total }) => {
  console.log(`Order ${orderId} placed for $${total}`);
});

// Emit
Emitter.emit<AppEvents, 'order.placed'>('order.placed', { orderId: 'abc-123', total: 49.99 });

// Remove a listener
Emitter.off<AppEvents, 'order.placed'>('order.placed', handler);
```
...
The built-in auth event handlers intentionally demonstrate non-duplicative side effects: `user.registered`, `user.login`, and `user.verified` are logged. Registration email delivery remains in `AuthController` so emitting `user.registered` does not enqueue a second welcome email.

## Cache

A simple key/value cache backed by an in-process driver. The framework runtime wires `MemoryCache` by default in `src/runtime/config.ts`.
...
Cache.setDriver(new RedisCacheDriver());
```

## Storage

File storage backed by a driver facade. The framework runtime explicitly wires `LocalDiskDriver` in `src/storage/config.ts` via `src/runtime/config.ts`; it does not switch storage drivers from environment variables. The local driver writes files under the `storage/` directory by default and serves them at `/storage/<path>`.
...
| ------- | ------------------------------------------------- | ------------------------------------------ |
| `local` | Writes to disk under `storage/` by default | Wired by `src/storage/config.ts` |
| `s3`    | AWS S3 (or S3-compatible services like MinIO)     | Available when your application explicitly injects `S3Driver` |
...
Storage.setDriver(new CloudinaryDriver());
```
...
| `events.spec.ts` | `Emitter.on / emit / off`, app-owned typed events via `src/events/events.ts` |
| `mail.spec.ts` | `Mailer.send` with injected drivers and default sender handling |
