...
│   └── orm.config.ts
├── events/          # Event surface and listeners
├── middleware/      # auth, inertia, rateLimit, errorHandler
├── models/          # Plain TS classes (no decorators)
├── primitives/      # Mail, storage, queue, cache, scheduler, bus, inertia helpers
├── router/          # app.ts + route.ts
├── utils/           # Hash, etc.
├── views/
...
└── index.ts         # Server process startup
```
...
This creates `src/controllers/posts.ts`, `src/views/pages/Posts.tsx`,
...
export async function index(req: Request, res: Response) {
  ...
  return renderPage(req, res, 'Posts', { posts });
}
...
export async function show(req: Request, res: Response) {
  ...
  return renderPage(req, res, 'Post', { post });
}
...
- `renderPage(req, res, 'PageName', props, { title })` is the canonical way to send an Inertia response. The page name is type-checked against `PageName`.
...
Integration tests live in `test/integration/` and use Vitest + supertest against the real Express app and a real SQLite database.
...
| ------ | --------- | --------------- |
| `log`  | Wired by default in bootstrap | None — writes to Pino logger |
| `smtp` | When you switch to it in bootstrap | `MAIL_HOST`, `MAIL_PORT`, `MAIL_USER`, `MAIL_PASS`, `MAIL_FROM` |

### Registering a custom driver

```ts
import { Mailer, MailTransport, MailMessage } from '../primitives/mail';
...
```ts
// src/scheduler/cleanExpiredSessions.ts
import { Scheduler } from '../primitives/scheduler';

Scheduler.on('0 3 * * *', cleanExpiredSessions);
```

Run schedules through the background worker process with `npm run work`.

### API

| Method | Description |
| ------ | ----------- |
| `Scheduler.on(expr, fn)` | Register a cron task (validates the expression) |
| `Scheduler.schedule(expr, fn)` | Alias for `Scheduler.on(expr, fn)` |
| `Scheduler.start()` | Start all registered tasks |
| `Scheduler.stop()` | Stop all registered tasks |
...
A lightweight typed event bus backed by Node's `EventEmitter`.
...
Bus.on('auth.registered', ({ id, email }) => {
...
| ---------------- | ------------------------------- | ----------------------------------- |
| `auth.registered` | `{ id, email }`                | User signs up                       |
| `auth.loggedIn`   | `{ id, email }`                | User signs in                       |
| `auth.verified`   | `{ id, email }`                | User verifies their email address   |

### API

| Method | Description |
| ------ | ----------- |
| `Bus.publish(event, payload)` | Publish an event |
| `Bus.on(event, listener)` | Subscribe to an event |
...
```ts
import { Storage, StorageDriver } from '../primitives/storage';
...
class CloudinaryDriver implements StorageDriver {
  ...
  url(filePath: string) { return `https://cdn.example.com/${filePath}`; }
