...
make_controller() {
	...
	local file_name
	file_name="$(kebab "$name")"
	local file="$CTRL_DIR/${file_name}.ts"
	if [ -e "$file" ]; then
		info "controller exists: ${file_name}.ts"
		return
	fi
	cat > "$file" <<TS
...
TS
	green "created src/controllers/${file_name}.ts"
}
...
add_route() {
	...
	local ctrl_file
	ctrl_file="$(kebab "$ctrl")"

	ensure_import "import * as ${ctrl} from '@/controllers/${ctrl_file}';"
	...
	local line="route.${method}('${url}'${middleware}, ${ctrl}.${action});"
...
make_event_listener() {
	...
	local camel
	camel="$(printf '%s' "$name" | awk '{print tolower(substr($0,1,1)) substr($0,2)}')"
	local file="$EVENTS_DIR/${camel}.ts"
	if [ -e "$file" ]; then
		info "event exists: ${camel}.ts"
		return
	fi
	cat > "$file" <<TS
import { Bus } from '@/primitives/bus';

// TODO: replace 'auth.registered' with the event you want to listen for
Bus.on('auth.registered', (payload) => {
	// TODO: implement ${name} event logic
	void payload;
});
TS
	green "created src/events/${camel}.ts"
}
